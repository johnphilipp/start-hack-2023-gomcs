...
const LoggedIn: React.FC = () => {
  ...
  const handlePopulateDashboard = async () => {
    if (sessionData && uploadedFile) {
      try {
        const response = await populateDashboard(
          sessionData.user.id,
          uploadedFile
        );
    ...
    <div className="flex flex-col items-center justify-center gap-4">
      ...
      {uploadedFile && sessionData && (
        <button
          ...
          onClick={() => handlePopulateDashboard()}
        >
      ...
      {success && (
        <p className="text-green-500">Dashboard populated successfully!</p> // conditionally render success message
      )}
    </div>
  );
};
...
const populateDashboard = async (userId: string, timeline: string) => {
